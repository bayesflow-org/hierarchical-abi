...
# make dir for plots
if not os.path.exists(f"experiments/plots/{score_model.name}"):
    os.makedirs(f"experiments/plots/{score_model.name}")
#%%
...
fig.savefig(f'experiments/plots/{score_model.name}/recovery_global.pdf')
...
fig.savefig(f'experiments/plots/{score_model.name}/ecdf_global.pdf')
#%%
...
fig.savefig(f'experiments/plots/{score_model.name}/recovery_local.pdf')
#
#%% Real data
...
binned_data = np.load('experiments/problems/FLI/exp_binned_data.npy')[:grid_data, :grid_data]
binned_data = binned_data.reshape(1, grid_data * grid_data, 256, 1)

data = np.load('experiments/problems/FLI/final_Data.npy')[:, :grid_data, :grid_data]
...
for j, real_data in enumerate([binned_data, data]):
    ...
    fig.savefig(f'experiments/plots/{score_model.name}/real_data_global_posterior_{j}.pdf')

    fig = diagnostics.pairs_posterior(
        posterior_tranf_dict,
        priors=prior_tranf_dict,
    )
    fig.savefig(f'experiments/plots/{score_model.name}/real_data_global_posterior_transf_{j}.pdf')
    ...
    visualize_simulation_output(med,
                                ...
                                cmap='jet', save_path=f"experiments/plots/{score_model.name}/real_data_median_{j}.pdf")
    visualize_simulation_output(posterior_mad,
                                mask=binary_mask.reshape(grid_data, grid_data), add_scale_bar=True,
                                title_prefix=['Posterior MAD ' + p for p in transf_local_param_names],
                                cmap='jet', save_path=f"experiments/plots/{score_model.name}/real_data_mad_{j}.pdf")
    visualize_simulation_output(med,
                                mask=binary_mask.reshape(grid_data, grid_data),
                                title_prefix=['Posterior Median ' + p for p in transf_local_param_names],
                                scales=[(0,1), (0, 2), (0,1), (0, 2)], add_scale_bar=True,
                                cmap='jet', save_path=f"experiments/plots/{score_model.name}/real_data_median_scales_{j}.pdf")
    visualize_simulation_output(posterior_mad,
                                mask=binary_mask.reshape(grid_data, grid_data),
                                title_prefix=['Posterior MAD ' + p for p in transf_local_param_names],
                                scales=[(0,1), (0, 2), (0,1), (0, 2)], add_scale_bar=True,
                                cmap='jet', save_path=f"experiments/plots/{score_model.name}/real_data_mad_scales_{j}.pdf")
    np.save(f'experiments/plots/{score_model.name}/fli_local_median_{j}', med)
    np.save(f'experiments/plots/{score_model.name}/fli_local_mad_{j}', posterior_mad)
