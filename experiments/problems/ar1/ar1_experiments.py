...
from experiments.problems import AR1GridPrior, AR1GridProblem
...
score_model = HierarchicalScoreModel(
    ...
    prior=prior,
    dropout_rate=0.1,
    name_prefix=f'test_ar1_{model_id}_{max_number_of_obs}'
)
import time
start = time.time()
# make dir for plots
...
data_sizes = np.array([4*4, 16*16, 64*64, 128*128])
